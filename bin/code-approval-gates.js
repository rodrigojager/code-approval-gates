...
function runQualityGate(workspace,outputDir,options,mode){
  ...
  const result=spawnSync(process.execPath,args,{cwd:workspace,encoding:"utf8",errors:"replace",timeout:Number(options.qualityTimeoutMs||0)||undefined,stdio:mode.headless?"pipe":"inherit"});
...
async function handleDoctor(cwd,parsed){
  ...
  if(["all","quality"].includes(focus)){
    checks.push(checkCommand("docker",["version","--format","{{.Server.Version}}"],"docker-daemon"));
    checks.push(checkDockerImage("code-approval-gates/quality-sidecar:latest"));
