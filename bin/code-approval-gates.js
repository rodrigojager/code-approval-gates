...
function fixCodexNetworkAccess(cwd,options){
  ...
  if(!isWindowsAdmin()){
    if(options.elevatedChild)return[{name:"codex-api-firewall-admin",status:"ERROR",message:`Elevation did not grant Administrator rights. Open PowerShell as Administrator and run: ${manualDoctorCommand(cwd,options)}`}];
    return runElevatedCodexNetworkRepair(cwd,options);
  }
...
function runElevatedCodexNetworkRepair(cwd,options){
  const command=manualDoctorCommand(cwd,options);
  const childArgs=elevatedDoctorArgs(cwd,options);
  const argumentLine=commandForDisplay(childArgs[0],childArgs.slice(1));
  const script=[
    "$ErrorActionPreference='Stop'",
    `$file=${psSingleQuote(process.execPath)}`,
    `$arguments=${psSingleQuote(argumentLine)}`,
    "$process=Start-Process -FilePath $file -ArgumentList $arguments -Verb RunAs -Wait -PassThru",
    "Write-Output (\"elevated exitCode=\" + $process.ExitCode)",
    "exit $process.ExitCode"
  ].join(";");
  const result=spawnSync("powershell.exe",["-NoProfile","-ExecutionPolicy","Bypass","-Command",script],{encoding:"utf8",errors:"replace",timeout:300000,windowsHide:true});
  if(result.status===0){
    return[
      {name:"codex-api-firewall-admin",status:"FIXED",message:`Elevated firewall repair completed via UAC. ${trimLog(result.stdout||"")}`},
      checkCodexApiNetwork(),
    ];
  }
  return[{name:"codex-api-firewall-admin",status:"ERROR",message:`Could not complete elevated firewall repair (${trimLog(result.stderr||result.stdout||result.error?.message||"UAC elevation failed or was cancelled")}). Run manually from Administrator PowerShell: ${command}`,exitCode:result.status??1}];
}
function isWindowsAdmin(){
  const result=spawnSync("net",["session"],{stdio:"ignore",windowsHide:true,timeout:5000});
  return result.status===0;
}
function elevatedDoctorArgs(cwd,options){
  const args=[__filename,"--cwd",cwd,"doctor","semantic","--fix-network","--yes","--elevated-child"];
  if(options.json)args.push("--json");
  if(options.noInteractive||options.json||options.ci)args.push("--no-interactive");
  return args;
}
function manualDoctorCommand(cwd,options){
...
function elevatedDoctorCommand(cwd,options){
  const args=[process.execPath,...elevatedDoctorArgs(cwd,options)];
  return commandForDisplay(args[0],args.slice(1));
}
...
Fix behavior:
  ...
  --fix-network     Windows admin repair for Codex/API outbound access: auto-elevates through UAC when needed, creates outbound TCP 443 firewall rules for the Codex/Node runtime, and checks api.openai.com.
